...
func _react(_action: String, _args):
	...
	if(_action == "runExport"):
		...
		for slot in template:
			...
			if( #save data format for 0.1.10 or higher
				GlobalRegistry.game_version_major > 0 ||
				GlobalRegistry.game_version_minor > 1 ||
				GlobalRegistry.game_version_revision >= 10
			):
				if("skin" in template[slot]["data"]):
					bodypart["pickedSkin"] = template[slot]["data"]["skin"]
				for color in ["r", "g", "b"]:
					if template[slot]["data"].has(color):
						if template[slot]["data"][color] != null:
							bodypart["picked"+color.to_upper()] = Color(template[slot]["data"][color])
						else:
							bodypart["picked"+color.to_upper()] = null
			else: #save data format for 0.1.9 or lower			
